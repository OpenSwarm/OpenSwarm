...
Sys_RawMessageList *Sys_convert_WordToPackage(uint8 address, uint32 dvalue, uint8 type){
    ...
    uint16 buffer = (((uint16) sys_com_device.msg_id+1) & 0x007F);
    sys_com_device.msg_id = (sys_com_device.msg_id+1) & 0x7E;
    
    uint16 as = sys_com_device.address;
    as = sys_com_device.address;
    buffer |= ((uint16) (sys_com_device.address & 0x0F)) << 7;
    out->message[0] = encodeBCH(buffer);
    
    buffer  = ((uint16) (sys_com_device.address & 0x30)) >> 4;
...
/*{
    ...
    //uint16 buffer = 0x0001; Synch bit not in data
    uint16 buffer = ((uint16) (sys_com_device.msg_id+1));
    sys_com_device.msg_id = (sys_com_device.msg_id+1) & 0x7E;
