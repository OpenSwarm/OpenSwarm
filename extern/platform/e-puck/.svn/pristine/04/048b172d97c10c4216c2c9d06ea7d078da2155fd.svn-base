...
int e_get_acc(unsigned int captor)
{
	...
	switch(captor) {
            // Shift by 4 the value received from the sensor (16 bits) to get 12 bits value as with the analog accelerometer (to maintain compatibility with e-puck 1.0)
            // Moreover decrease the resulting value by 1/4 to get about the same resolution for the g:
            // - e-puck 1.0: 1 g = about 800
            // - e-puck 1.1: 1 g = about 1000 => decreased by 1/4 => about 750 (5% difference)
            case 0: tempValue = lastAccY - centre_y;   // exchange x and y axes in order to be consistent with e-puck 1.0
...
int e_get_acc_filtered(unsigned int captor, unsigned int filter_size)
{
    ...
    if(isEpuckVersion1_3()) {
        // Filter no more required for the new chip used with e-puck 1.1.
        ...
        if(isCalibratingFlag) {
            ...
            switch(captor) {
                ...
                // - e-puck 1.0: 1 g = about 800
                // - e-puck 1.1: 1 g = about 1000 => decreased by 1/4 => about 750 (5% difference)
                // Add an offset of 2048 because it represents the value of 0 g with the analog accelerometer (to maintain compatibility with e-puck 1.0).
                case 0: tempValue = lastAccY;  // exchange x and y axes in order to be consistent with e-puck 1.0;
