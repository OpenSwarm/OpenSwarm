...
void e_init_ad_scan(unsigned char only_micro)
{
	...
	if(selector == 10) {	// gumstix extension
		ADPCFGbits.PCFG0 = 0;   // ir8, right
		ADPCFGbits.PCFG1 = 0;   // ir9, left
	} else {
		ADPCFGbits.PCFG0 = 1;   // front and body leds
		ADPCFGbits.PCFG1 = 1;
	}
	...
	ADCSSLbits.CSSL0 = 0;   // Debugger or ir8 on gumstix ext.
	ADCSSLbits.CSSL1 = 0;   // Debugger or ir9 on gumstix ext.
...
void __attribute__((__interrupt__, auto_psv)) _ADCInterrupt(void)
{
	...
	IFS0bits.ADIF = 0;
        ...
        if(updateAccI2CCounter < 64) {  // 64*1/16384 = about 3.9 ms (256 Hz)
            updateAccI2CCounter++;      // tell when the i2c accelerometer (e-puck 1.1) need an update
        }
	...
	else
	{
			...
			if(selector == 10) { // gumstix ext. demo
				...
				{	
					...
					case 3:	ADCSSL=0x881C;
							break;
					// ir sensors 8 and 9
					case 4: ADCSSL=0x0003; //0x001F => doesn't work in this way (taking IRs+MIC)...why?
							break;
				}
			} else {
				ADCON2bits.SMPI = 5-1;	//interupt on 5th sample
		...
		if(prox_number < 4) {
			...
			*adc_ptr++;
			if(period_counter == ACC_PROX_PERIOD-1) {
